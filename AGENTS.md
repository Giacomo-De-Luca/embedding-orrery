...
ORRERY_IMAGE_NAMESPACE=<dockerhub-user> docker compose -f docker-compose.yml -f docker-compose.hub.yml up -d
...
- `ORRERY_SEED_REPO_ID` — private Hugging Face Dataset repo used when publishing the large demo seed
- `ORRERY_IMAGE_NAMESPACE` / `ORRERY_IMAGE_TAG` — Docker Hub image owner and release tag for the Hub Compose override
...
**Seed dataset (ships in git)**: The live data stores are gitignored, so a fresh clone would be empty. A small (~23 MB) committed seed snapshot at `interpretability_backend/resources/seed/` (`main.duckdb` + `vector_db/`) holds two demo collections — `emotion` (1000 rows, MiniLM, Gemini topic labels; semantic search works offline) and `xkcd_hilbert_gemini` (954 rows, Gemini-embedded XKCD colors with rainbow `mapped_colour` coloring). On first backend startup, `backend/utils/seed_bootstrap.py::ensure_seed_loaded()` (called from the FastAPI `lifespan` in `main.py`) copies the seed into the live paths **only if `main.duckdb` is absent** — it never clobbers an existing DB. Seeds are regenerated from versioned JSON manifests in `interpretability_backend/config/seed_snapshots/`; the builder stages and verifies checksums before atomic replacement. The large `seed_demo` remains gitignored, publishes to a private Hugging Face Dataset, and is consumed via the immutable generated `demo.lock.json`. See `documentation/SEED_SNAPSHOTS.md`. Note: live semantic search on the XKCD collection needs `GEMINI_API_KEY` (its vectors are Gemini); viewing/coloring/topics work with no model since projections are precomputed.

**Production Docker demo**: `docker-compose.yml` builds separate backend/frontend images plus a stock nginx same-origin gateway on port 3000 and stores mutable data in named volumes. `docker-compose.hub.yml` resets the build definitions and pulls versioned Docker Hub images. `docker compose --profile sae up --build` adds the cache-only `sae-warmup` service without calling `loadModel`. GitHub Actions publishes multi-architecture `edge` images from `main` and semantic-version/`latest` images from releases, then updates the locked-seed Hugging Face Space. Local npm development uses the backend on `localhost:8000`, while explicit Docker endpoint values select the gateway. Full endpoint and container behavior is documented in `documentation/DOCKER.md`; seed publication is in `documentation/SEED_SNAPSHOTS.md`.
...
**Memory-aware projection loading**: Explore initially requests only the active `{method}_{dimension}` projection plus core item data. Later method/dimension changes use `includeCore: false` and return coordinates plus a compact ordered-item signature; the frontend validates that signature before merging. `VisualizationPointBuilder` creates wrappers only for the active dimension, while `PointTraceColumns3D` supplies Float32 coordinate columns and Uint32 indices to Plotly data traces. The GraphQL argument remains backward compatible with `includeCore: true` by default. The headed-Chrome regression harness writes labeled artifacts and records GraphQL bytes plus forced-GC heap without overwriting the July 2026 baselines; see `benchmarks/fps/README.md`. See `documentation/FRONTEND_HEAP_REDUCTION.md` for the request lifecycle, tests, and remaining roadmap work.
