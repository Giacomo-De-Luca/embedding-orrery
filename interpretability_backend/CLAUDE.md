...
- **`seed_bootstrap.py`** - `ensure_seed_loaded()`: on first run copies the seed snapshot (`ORRERY_SEED_DIR`, default `resources/seed/`) into the live paths, **only if `main.duckdb` is absent** (never clobbers an existing DB). Called from the FastAPI `lifespan` hook in `main.py`. Build/refresh seeds with `scripts/build_seed_snapshot.py` (run with the backend stopped — DuckDB is single-writer). Parameterized via `--collections/--datasets/--output` (defaults reproduce the committed seed); copies use `INSERT … BY NAME` (production tables gain columns via guarded ALTERs, so positional `SELECT *` breaks against a fresh schema), Chroma vectors copy in 5k batches, and an empty production Chroma collection falls back to the committed seed as vector source (live gap: production `emotion` has 0 vectors). The committed seed ships `emotion` + `xkcd_hilbert_gemini` (~23 MB); the gitignored `resources/seed_demo/` (~313 MB) adds the EMNLP collection for the HF Space demo (`documentation/HF_SPACE_DEMO.md`).
