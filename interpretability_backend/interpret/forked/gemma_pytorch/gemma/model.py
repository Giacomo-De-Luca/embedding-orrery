...
class Gemma2DecoderLayer(nn.Module):
    ...
    def _cache(
        self,
        activation_cache: dict | None,
        cache_intermediates: set[str] | None,
        name: str,
        tensor: torch.Tensor,
    ) -> None:
        """Store an activation in the cache if this layer and intermediate are targeted."""
        if (
            activation_cache is not None
            and self.layer_idx in activation_cache
            and name in cache_intermediates
        ):
            activation_cache[self.layer_idx][name] = tensor.detach().clone().cpu()
    ...
    def forward(
        ...
        cache_intermediates: set[str] | None = None,
    ) -> torch.Tensor:
        # Self Attention
        residual = hidden_states
        self._cache(activation_cache, cache_intermediates, "pre_attn", hidden_states)
        hidden_states = self.input_layernorm(hidden_states)
        hidden_states = self.self_attn(
            hidden_states=hidden_states,
            freqs_cis=freqs_cis,
        ...
        hidden_states = self.mlp(hidden_states)
        self._cache(activation_cache, cache_intermediates, "mlp_out", hidden_states)
        if self.post_feedforward_layernorm is not None:
            hidden_states = self.post_feedforward_layernorm(hidden_states)
        hidden_states = residual + hidden_states
        self._cache(activation_cache, cache_intermediates, "post_mlp", hidden_states)
...
class GemmaModel(nn.Module):
    ...
    def __init__(self, config: gemma_config.GemmaConfig):
        ...
        # Activation cache state (configured via configure_cache)
        self.cache_layers: set[int] | None = None
        self.cache_intermediates: set[str] | None = None
        self.cache_prefill: bool = False
        self.cache_last: bool = False
        self._step_cache: dict[int, dict[str, torch.Tensor]] = {}
        self._step_final_norm: torch.Tensor | None = None
        self._prefill_cache: dict[int, dict[str, torch.Tensor]] = {}
        self._prefill_final_norm: torch.Tensor | None = None
        self._last_cache: dict[int, dict[str, torch.Tensor]] = {}
        self._last_final_norm: torch.Tensor | None = None
        self._prefill_captured: bool = False
        ...
        for i in range(config.num_hidden_layers):
            if config.architecture == gemma_config.Architecture.GEMMA_1:
                ...
                self.layers.append(Gemma2DecoderLayer(config, attn_type, layer_idx=i))
            else:
    ...
    def configure_cache(
        ...
        """Configure which layers, intermediates, and generation steps to cache.
        ...
        Args:
            ...
            last: Capture activations from the most recent forward call.
                Updated every step; after generation, holds the final decode step.
        """
        import warnings
        if not prefill and not last:
            raise ValueError("At least one of prefill or last must be True.")
        num_layers = len(self.layers)
        if intermediates is not None:
            invalid = intermediates - self.VALID_INTERMEDIATES
            if invalid:
                raise ValueError(
                    f"Invalid intermediates: {invalid}. "
                    f"Valid options: {self.VALID_INTERMEDIATES}"
                )
        resolved_layers = layers if layers is not None else set(range(num_layers))
        out_of_range = {i for i in resolved_layers if i >= num_layers}
        if out_of_range:
            warnings.warn(
                f"Layer indices {out_of_range} exceed model depth ({num_layers} layers), ignoring.",
                stacklevel=2,
            )
        self.cache_layers = resolved_layers - out_of_range
        self.cache_intermediates = intermediates if intermediates is not None else set(self.VALID_INTERMEDIATES)
        self.cache_prefill = prefill
        self.cache_last = last
        self._step_cache = {i: {} for i in self.cache_layers}
        self._step_final_norm = None
        self._prefill_cache = {}
        self._prefill_final_norm = None
        self._last_cache = {}
        self._last_final_norm = None
        self._prefill_captured = False

    def clear_cache(self) -> None:
        """Disable caching and release stored activations."""
        self.cache_layers = None
        self.cache_intermediates = None
        self.cache_prefill = False
        self.cache_last = False
        self._step_cache = {}
        self._step_final_norm = None
        self._prefill_cache = {}
        self._prefill_final_norm = None
    ...
    def _snapshot_step_cache(self) -> tuple[dict[int, dict[str, torch.Tensor]], torch.Tensor | None]:
        """Take a shallow copy of the current step cache (tensors are already independent)."""
        return {i: dict(v) for i, v in self._step_cache.items()}, self._step_final_norm
    ...
    def forward(
        ...
        caching = self.cache_layers is not None
        cache = self._step_cache if caching else None
        cache_intermediates = self.cache_intermediates

        # Reset per-layer step cache
        if caching:
            for i in self._step_cache:
                self._step_cache[i] = {}
        ...
        if caching and cache_intermediates and "final_norm" in cache_intermediates:
            self._step_final_norm = hidden_states.detach().clone().cpu()

        # Snapshot to prefill (first call only) and/or last (every call)
        if caching:
            if self.cache_prefill and not self._prefill_captured:
                self._prefill_cache, self._prefill_final_norm = self._snapshot_step_cache()
                self._prefill_captured = True
            if self.cache_last:
                self._last_cache, self._last_final_norm = self._snapshot_step_cache()
