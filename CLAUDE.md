...
| HF Space demo | Read-only public demo from the root `Dockerfile` (nginx → uvicorn + Next standalone); backend `ORRERY_READ_ONLY=1`, frontend `NEXT_PUBLIC_DEMO_MODE=1`, seed pinned by `demo.lock.json`, deployed via GitHub Actions. Onboarding: welcome dialog + `?preset=`/`?tour=` URL layer + react-joyride spotlight tour, with parent-page URL sync inside the Space iframe. Social link cards: one 1200×630 image (`embedding_visualization/app/opengraph-image.jpg`) serves both the Space card (README `thumbnail`, Hub-served) and the direct host's Open Graph tags (`metadataBase` from the Space Variable `NEXT_PUBLIC_SITE_URL`, pinned by `deploy.py`). | `documentation/HF_SPACE_DEMO.md` ("Onboarding", "Social preview"), `documentation/DOCKER.md`, frontend claude.md "Demo onboarding" |
