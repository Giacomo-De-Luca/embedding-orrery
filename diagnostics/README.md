...
| `qwen_scope_smoke.py` | End-to-end smoke test for the Qwen-scope TopK SAE path: shape / dtype / sparsity (per-token L0 == k) / reconstruction-cosine / additive-steering checks against `Qwen/SAE-Res-Qwen3-1.7B-Base-W32K-L0_50`. |

Run with:

```
uv run python -m interpret.sae.diagnostics.check_feature_index_alignment
uv run python -m interpret.sae.diagnostics.smoke_steering_pirate
uv run python -m interpret.sae.diagnostics.debug_steering_injection
