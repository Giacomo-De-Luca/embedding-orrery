...
The platforms that succeeded the Projector — Nomic Atlas, WizMap, Latent Scope, Embedding Atlas — solved the second task, adding clustering and automated topic labelling at the scale of millions of points; but they reverted to 2D and fixed, minimally configurable pipelines, favouring scalability over navigation (§2). The third task, finding directions in the original space, remains crucial in interpretability research and has, to our knowledge, not been addressed by any platform: linear structure — factual truth, refusal — emerges unsupervised in representation spaces, can be recovered by simple probes `[Marks2023TheGO]`, and doubles as a handle for causal intervention `[arditi2024refusallanguagemodelsmediated]`. Meanwhile, Sparse Autoencoders (SAEs) have turned model internals themselves into vector collections — each decoder row is a vector in representation space, so feature spaces can be mapped like any corpus — yet existing feature maps are one-off exhibits, not navigable platforms (§2).

Orrery addresses both gaps at once: (a) it makes the Anthropic-style feature map available as open source, backed by the same exploration machinery as any embedding collection; (b) it improves on the feature dashboard by fusing it with map-level exploration and direct steering of the model; and (c) because corpora and feature spaces are the same kind of object in the platform, it completes, in-platform, all three of the Projector's tasks — clusters via configurable topic extraction, neighbourhoods via click-to-search in the original space, and directions via analytical colouring, in-interface probing (§5.1), and causal steering — extending the third task from embedding space into model internals. The intended workflow is: *find a semantic pattern → identify candidate features or directions → inspect the evidence → intervene in the model → observe the behavioural effect.*
...
**Embedding-visualisation platforms.** The Embedding Projector rendered embeddings as 3D scatter plots reduced with PCA or t-SNE `[van2008visualizing]`; Google's later explorations of 3D vector spaces were artistic experiments — the *t-SNE Map* for images and *Semantic Galaxy* for Gemma embeddings **[TODO: verify attribution — the HF space is published by webml-community]** — demos limited in scope rather than general platforms (Semantic Galaxy is the main inspiration behind Orrery's cosmic theme). When UMAP was released `[mcinnes2018umap]`, its effectiveness was itself demonstrated through scatter plots showing cleaner clustering than t-SNE. The post-LLM generation of platforms all made the same two choices: **2D, and scale**. Nomic Atlas visualises millions of embeddings with automatically generated clusters and labels, plus filtering and search over the collection; WizMap `[wang2023wizmapscalableinteractivevisualization]` likewise renders millions of points, using density contouring to find regions of interest; Latent Scope offers an open-source local pipeline (embed → project → cluster → label) with early, limited SAE support **[TODO: characterise precisely]**; Embedding Atlas `[ren2025embeddingatlaslowfrictioninteractive]` is the most polished of the recent generation. Alongside the retreat from 3D, all offer restricted clustering, projection, and labelling options — typically one algorithm each, minimally configurable. Orrery brings the focus back to 3D and *exploration*, with projection (PCA/UMAP, 2D/3D), clustering (HDBSCAN, K-means, GMM, spectral, over a configurable space), and labelling (c-TF-IDF, LLM, hierarchical reduction) as first-class, configurable steps.

**SAE feature visualisation.** *Towards Monosemanticity* `[bricken2023monosemanticity]` already contains the visualisation framework still in use for SAE features: an elegant per-feature dashboard — activation density, top and bottom logits, activation examples — and, alongside it, a 2D UMAP scatter plot of all 4,608 features. The follow-up, *Scaling Monosemanticity* `[templeton2024scaling]`, displays a striking map of the feature space of Claude 3 Sonnet, but both interfaces remain closed-source and non-replicable. Open-source tooling — SAE-Vis, its successor SAEDashboard, and Neuronpedia `[neuronpedia]` — faithfully replicated the dashboard while **leaving the map behind**: Neuronpedia exposes a 2D UMAP for GPT-2 Small only, with no neighbour search, clustering, or labelling, and Google's PAIR published a pedagogical explorable of a single Gemma SAE (pair.withgoogle.com/explorables/sae). Existing feature maps are one-off exhibits of a fixed model and SAE; none is a navigable platform where a feature space can be searched, clustered, and labelled like any other embedding collection.

---

## 3 System Design

### 3.1 Datasets and collections
...
Orrery treats an SAE's feature space as itself an embeddable dataset — restoring the feature-space map that open-source SAE tooling left behind (§2) and closing the loop between corpus geometry and model internals. A dedicated collection type, the *SAE collection*, can be created with one click: features, labels, and decoder vectors for any Gemma Scope SAE are downloaded from the Neuronpedia S3 bucket and ingested into DuckDB/ChromaDB.

### 4.1 Visualising the feature space

A feature space can be constellated in two ways: (i) using each feature's row of the **decoder matrix** (dimension = the model's residual stream) as its vector, or (ii) **embedding the auto-interpreted label** of each feature with a conventional text encoder. Either yields a `{feature_index, label, vector, top_logits, exemplar documents}` dataset that flows through the standard projection pipeline. SAE collections gain two abilities beyond normal collections: prompt search (§3.3) — each point *is* a feature, so the features most activated by a prompt are highlighted and ranked — and right-click navigation from any feature-point to its full dashboard.

### 4.2 Feature explorer
...
The case studies instantiate the Projector's third, never-supported task — *finding meaningful directions* — at three levels: linear directions in embedding space (§5.1), nonlinear perceptual structure that projection makes visible (§5.2), and a causal direction inside the model (§5.3). §5.4 exercises the other two tasks (clusters, neighbourhoods) at corpus scale.

### 5.1 Psycholinguistic norms are linear directions
...
### 5.2 XKCD colour survey: projections can *reveal* structure
...
### 5.3 Refusal direction on Gemma-3-4b-it
...
### 5.4 Digital humanities at scale
...
| SAE feature-space map (navigable) | ● | – | – | – | – | – | partial¹ |
| SAE steering + chat | ● | – | – | – | – | – | – |
...
¹ Neuronpedia's 2D UMAP covers GPT-2 Small only, with no neighbour search, clustering, or labelling; Google PAIR's SAE explorable is a single fixed Gemma SAE.
...
**Resolved 2026-07-06:** "only Anthropic released SAE feature maps" / "no open-source counterpart" was too strong. Author-verified counterexamples, now named in the intro and table footnote: (a) Neuronpedia has a 2D UMAP but GPT-2 Small only, no neighbour search / clustering / labelling; (b) Google PAIR's SAE explorable (pair.withgoogle.com/explorables/sae) is a single fixed Gemma SAE, pedagogical, not navigable. Claim reframed as: existing maps are one-off exhibits, none is a navigable platform.
